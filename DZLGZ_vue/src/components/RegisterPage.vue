    ...
    <div class="register-container">
      ...
      <form @submit.prevent="register">
        <div class="input-group">
          ...
          <input type="text" id="username" name="username" v-model="username" @focus="hideLabel('username')" @blur="showLabel('username')" />
        </div>
        <div class="input-group">
        ...
        <div class="input-group">
          ...
          <input type="password" id="confirm-password" name="confirm-password" v-model="confirmPassword" @focus="hideLabel('confirmPassword')" @blur="showLabel('confirmPassword')" />
        </div>
...
.input-group {
  margin-bottom: 0.1rem; /* 减少输入框之间的距离 */
  position: relative;
  ...
  padding: 1.0rem 2rem; /* 添加内边距以确保输入框不贴边 */
}
...
label {
  ...
  top: 50%;
  left: 0.8rem; /* 调整左边距 */
  transform: translateY(-35%); /* 使标签垂直居中 */
  pointer-events: none; /* 使标签不可点击 */
  z-index: 1; /* 确保标签在输入框上层 */
}

input {
  width: 30rem; /* 增大输入框的宽度 */
  border: none;
  ...
  transform: translate(-1.8rem, 0.4rem); /* 向左和向下调整位置 */
}
